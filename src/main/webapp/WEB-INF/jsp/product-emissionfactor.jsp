...
<head>
    <meta charset="UTF-8"/>
    <title>Product Emission Factor Lookup</title>
    <style>
...
<h1>Product Emission Factor Lookup</h1>
<p class="lead">Fetch emission factors for a product via <code>/v1/organization/products/{productUid}/emissionfactors</code>.</p>
...
<form method="post" action="/product-emissionfactors/query">
    <p>Enter the product UID, then optionally filter by year.</p>

    <label for="productUid">Product UID</label>
    <input type="text"
           id="productUid"
           name="productUid"
           value="${emissionFactorProductUidFieldValue}"
           placeholder="ex: prod_xid_12345"
           required/>
    ...
    <input type="text"
           id="year"
           name="year"
           value="${emissionFactorYearFieldValue}"
           placeholder="optional numeric year"/>

    <button type="submit">Query Emission Factors</button>

    <c:if test="${emissionFactorLookupMessage ne null}">
        <div class="status success">${emissionFactorLookupMessage}</div>
    </c:if>
    <c:if test="${emissionFactorLookupError ne null}">
        <div class="status error">${emissionFactorLookupError}</div>
    </c:if>
</form>

<c:if test="${emissionFactorLookupResult ne null}">
    <pre>${emissionFactorLookupResult}</pre>
</c:if>
</body>
</html>
